...
- **Godot 4.5.1 (.NET/mono build)** — required only for `dotnet publish` (asset/.pck export). MegaDot (MegaCrit's Godot fork) works too; what matters is the **4.5.1 version match** — the game won't load a `.pck` exported by a newer Godot. Standard builds: [godotengine.org/download/archive](https://godotengine.org/download/archive/) (pick “.NET”).
