...
using TheAlchemist.TheAlchemistCode.Cards;

namespace TheAlchemist.TheAlchemistCode.Commands;

public static class AlchemistCardCmd
{
    public static async Task GiveCard<T>(TheAlchemistCard source) where T : CardModel, new()
    {
    ...
    public static async Task AddStatus<T>(TheAlchemistCard source) where T : CardModel, new()
    {
        if (source.CombatState == null) return;
        var card = source.CombatState.CreateCard<T>(source.Owner);
    ...
    public static async Task TransformFromHand<T>(
        PlayerChoiceContext choiceContext, TheAlchemistCard source) where T : CardModel, new()
    {
        if (source.CombatState == null) return;
    ...
    public static async Task PoisonAll(
        PlayerChoiceContext choiceContext, TheAlchemistCard source)
    {
        if (source.CombatState == null) return;
