...
- **Every bridge-driven run needs Profile 3, including a manual run.** The suite selects
  the profile itself, but `bridge_start_run` and console fixtures use the profile that is
  active in the game. Confirm the active profile before you start a run by hand. A test
  run on a real profile pollutes the Run History that the balance reviews read.
