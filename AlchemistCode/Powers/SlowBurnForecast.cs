...
public sealed class SlowBurnForecast : IHealthBarForecastSource
{
    public IEnumerable<HealthBarForecastSegment> GetHealthBarForecastSegments(HealthBarForecastContext context)
    {
        ...
        if (context.Creature.GetPower<SlowBurnPower>() is not { IsArmed: true })
            yield break;

        // The capped number, so the bar matches the icon when Hard to Kill or Intangible applies
        var amount = context.Creature.GetPower<SlowBurnPower>()!.EffectiveDamage;
        if (amount > 0)
