...
While pre-1.0 the mod is "feature-complete but not yet publicly released" —
...
3. Paste the relevant lines under `## [Unreleased]` and **reword them into
   player-facing language** — "feat: rework infuse enchantments" becomes
...
1. **Preflight** — refuse unless the working tree is clean and you're on `main`;
   run `dotnet build` and the `lint` check; refuse if `## [Unreleased]` is empty.
...
**Steam Workshop is the preferred way to install and play** — one click,
